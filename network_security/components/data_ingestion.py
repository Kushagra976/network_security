...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self):
        ...
        try:
            if not MONGO_DB_URL:
                raise ValueError("MONGO_DB_URL is not set. Add it to your environment or .env file before running data ingestion.")

            database_name=self.data_ingestion_config.database_name
            collection_name=self.data_ingestion_config.collection_name
            self.mongo_client=pymongo.MongoClient(MONGO_DB_URL, tlsCAFile=CA_FILE_PATH)
            collection=self.mongo_client[database_name][collection_name]

            df=pd.DataFrame(list(collection.find()))
            if df.empty:
                raise ValueError(
                    f"No records found in MongoDB collection '{collection_name}' from database '{database_name}'."
                )
    ...
    def split_data_into_train_test(self,dataframe:pd.DataFrame):
        try:
            ...
            logging.info("exited the split_data_into_train_test method of DataIngestion class")
